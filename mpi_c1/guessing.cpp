...
void PriorityQueue::Generate(PT pt)
{
    ...
    // 记录猜测数量增加了多少
    int original_count = guesses.size();
    ...
    // 更新总猜测计数器
    total_guesses += (guesses.size() - original_count);
}

// 新增的函数，处理多个PT并收集它们的猜测
void PriorityQueue::ProcessMultiplePTs(int batch_size)
{
    if (batch_size <= 0 || priority.empty())
        return;
    
    // 保存当前队列中需要处理的前batch_size个PT
    vector<PT> batch_pts;
    for (int i = 0; i < batch_size && i < priority.size(); i++) {
