...
void MD5HashBatch(const vector<string>& inputs, vector<bit32*>& states) {
    ...
    for(size_t base = 0; base < inputs.size(); base += SIMD_WIDTH) {
        ...
        for(int block = 0; block < blocks; block++) {
            ...
            uint32x4_t a = state_a;
            uint32x4_t b = state_b;
            uint32x4_t c = state_c;
            uint32x4_t d = state_d;
        ...
        vst1q_u32(result_a, state_a);
        vst1q_u32(result_b, state_b);
        vst1q_u32(result_c, state_c);
        vst1q_u32(result_d, state_d);
