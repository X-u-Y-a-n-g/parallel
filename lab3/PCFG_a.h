...
const vector<float> LEVEL_THRESHOLDS = {0.1, 0.01, 0.001, 0.0};
...
public:
    ...
    // 模型作为成员，辅助猜测生成
    model m;
    std::atomic<size_t> total_guesses{0};  // 修改为 size_t 类型
    // 计算一个pt的概率
    void CalProb(PT &pt);
    PriorityQueue() : queue_mutex(std::make_unique<mutex>()) {}
    mutex queue_mutex;
    int total_guesses = 0;
    ...
    bool empty() const { return priority.empty(); }

    PT pop_front();
    void merge_guesses(vector<string>& target);
    void clear_guesses() {  // 添加清除方法
        if(queue_mutex) {
...
private:
    ...
    vector<unique_ptr<mutex>> level_mutexes;  // 使用智能指针管理互斥锁

    model& m;
    std::atomic<size_t> total_guesses{0};
    vector<string> password_guesses;
    mutex queue_mutex;  // 直接使用 mutex

public:
    MultiLevelQueue(model& model) : m(model), queue_mutex(std::make_unique<mutex>()) {
        queues.resize(NUM_LEVELS);
        level_mutexes.resize(NUM_LEVELS);  // 初始化互斥锁向量
        for(auto& q : queues) {
            q.m = model;
        }
    }

    void distribute_pt(PT& pt);
    void process_parallel();
    void init();
    bool try_get_work(int& level, PT& pt);  // 添加方法声明

    size_t get_total_guesses() const { return total_guesses.load(); }
    const vector<string>& get_guesses() const { return password_guesses; }
};