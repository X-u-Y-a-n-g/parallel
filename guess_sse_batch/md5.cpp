...
vector<Byte*> StringProcessBatch(const vector<string>& inputs, vector<int>& messageLengths, size_t batchSize) {
    vector<Byte*> paddedMessages(batchSize);
    messageLengths.resize(batchSize);
    
    // 预先计算所有输入的长度和填充大小
    size_t maxPaddedLength = 0;
    vector<int> lengths(batchSize);
    vector<int> paddedLengths(batchSize);
    vector<int> paddingBytes(batchSize);
    
    // 第一遍扫描：计算所有长度
    for(size_t i = 0; i < batchSize; i++) {
        const string& input = i < inputs.size() ? inputs[i] : inputs[0];
        lengths[i] = input.length();
        
        int bitLength = lengths[i] * 8;
        int paddingBits = bitLength % 512;
        ...
        paddingBytes[i] = paddingBits / 8;
        paddedLengths[i] = lengths[i] + paddingBytes[i] + 8;
        maxPaddedLength = max(maxPaddedLength, (size_t)paddedLengths[i]);
    }
    
    // 一次性分配连续内存
    Byte* buffer = new Byte[maxPaddedLength * batchSize];
    
    // 第二遍：处理数据
    for(size_t i = 0; i < batchSize; i++) {
        const string& input = i < inputs.size() ? inputs[i] : inputs[0];
        paddedMessages[i] = buffer + i * maxPaddedLength;
        
        // 复制输入数据
        memcpy(paddedMessages[i], input.c_str(), lengths[i]);
        
        // 添加padding
        paddedMessages[i][lengths[i]] = 0x80;
        memset(paddedMessages[i] + lengths[i] + 1, 0, paddingBytes[i] - 1);
        
        // 添加长度信息（使用64位长度）
        uint64_t bitLen = (uint64_t)lengths[i] * 8;
        memcpy(paddedMessages[i] + paddedLengths[i] - 8, &bitLen, 8);
        
        messageLengths[i] = paddedLengths[i];
    }
...
void MD5HashBatch(const vector<string>& inputs, vector<bit32*>& states) {
	...
	vector<string> batchInputs;  // 改用vector<string>    // 按SIMD_WIDTH分组处理
	batchInputs.reserve(SIMD_WIDTH);  // 预分配空间

    for(size_t base = 0; base < inputs.size(); base += SIMD_WIDTH) {
        size_t remaining = min(SIMD_WIDTH, inputs.size() - base);
		batchInputs.clear();  // 清空之前的数据

		// 正确设置batchInputs
        for(size_t i = 0; i < remaining; i++) {
            batchInputs.push_back(inputs[base + i]);
        }
		// 如果不足SIMD_WIDTH个，用第一个输入填充
		while(batchInputs.size() < SIMD_WIDTH) {
			batchInputs.push_back(batchInputs[0]);
		}
        // 为当前批次分配状态
        for(size_t i = 0; i < remaining; i++) {
        ...
        // 使用StringProcessBatch处理当前批次
        vector<Byte*> paddedMessages = StringProcessBatch(batchInputs, messageLengths, SIMD_WIDTH);

        // 初始化SSE状态向量
		// 使用对齐的内存初始化SSE状态向量
		alignas(16) bit32 init_states[4][4] = {
		...
		for(int block = 0; block < blocks; block++) {
			...
			for(int j = 0; j < 16; j++) {
				for(int k = 0; k < 4; k++) {
					...
					memcpy(&temp[k], &paddedMessages[k][offset], 4);
				}
        ...
        // 清理本批次内存
        delete[] paddedMessages[0]; // 只需要删除第一个指针，
    }
}