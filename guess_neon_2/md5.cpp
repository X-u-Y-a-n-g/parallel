...
void MD5HashBatch(const vector<string>& inputs, vector<bit32*>& states) {
    ...
    for(size_t base = 0; base < inputs.size(); base += SIMD_WIDTH) {
        ...
        // 如果只有一个输入，用第一个消息填充第二个
        if(remaining == 1) {
            paddedMessages[1] = new Byte[messageLengths[0]];
            memcpy(paddedMessages[1], paddedMessages[0], messageLengths[0]);
            messageLengths[1] = messageLengths[0];
        }


        // 初始化SIMD状态向量
        uint32x2_t state_a = vcreate_u32((uint64_t)0x67452301 | ((uint64_t)0x67452301 << 32));
        uint32x2_t state_b = vcreate_u32((uint64_t)0xefcdab89 | ((uint64_t)0xefcdab89 << 32));
        uint32x2_t state_c = vcreate_u32((uint64_t)0x98badcfe | ((uint64_t)0x98badcfe << 32));
        uint32x2_t state_d = vcreate_u32((uint64_t)0x10325476 | ((uint64_t)0x10325476 << 32));
        ...
        for(int block = 0; block < blocks; block++) {
            ...
            uint32x2_t a = state_a;
            uint32x2_t b = state_b;
            uint32x2_t c = state_c;
            uint32x2_t d = state_d;
        ...
        // 保存最终结果
        uint32_t result_a[2], result_b[2], result_c[2], result_d[2];
        vst1_u32(result_a, state_a);
        vst1_u32(result_b, state_b);
        vst1_u32(result_c, state_c);
        vst1_u32(result_d, state_d);
